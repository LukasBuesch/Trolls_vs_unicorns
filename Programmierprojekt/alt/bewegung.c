...
void Sprung(struct Tasten *eingabe,struct Figur *spieler1, struct Figur *spieler2){
    ...
    if(((*eingabe).springenbewegungspieler1)==1&&(SDL_GetTicks()/100)>((*spieler1).zeitseitsprung)+SPRUNGVERZOEGERUNG){
        ...
        else if(((SDL_GetTicks()/100)-(*spieler1).zeitseitsprung)<=(SPRUNGZEIT)&&(*spieler1).positiony>((*spieler1).absprunghoehe-SPRUNGHOEHE))
        {
            (*spieler1).positiony=(*spieler1).positiony-8;
        }

       else if(((SDL_GetTicks()/100)-(*spieler1).zeitseitsprung)>(SPRUNGZEIT)&&(*spieler1).positiony<BODEN)
        {
        ...
        else if(((SDL_GetTicks()/100)-(*spieler2).zeitseitsprung)<=(SPRUNGZEIT)&&(*spieler2).positiony>((*spieler2).absprunghoehe-SPRUNGHOEHE))
        {
            (*spieler2).positiony=(*spieler2).positiony-8;
        }

       else if(((SDL_GetTicks()/100)-(*spieler2).zeitseitsprung)>(SPRUNGZEIT)&&(*spieler2).positiony<BODEN)
        {
